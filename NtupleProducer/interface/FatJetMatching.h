...
namespace deepntuples {
  ...
  public:
    ...
    void flavorLabel(const l1t::PFJet* jet,
  ...
  private:
    void top_label(const l1t::PFJet* jet,
    ...
    void qcd_label(unsigned int nBHadrons, unsigned int nCHadrons);
  ...
  private:
    ...
    double maxDeltaRToDaughterQuarks(const T* center, const reco::GenParticle* mother) const {
      ...
      for (const auto& q : mother->daughterRefVector()) {
        if (std::abs(q->pdgId()) > ParticleID::p_b)
          continue;
  ...
  private:
    ...
    FatJetMatchingResult result_{
        "Invalid", std::vector<const reco::GenParticle*>(), std::vector<const reco::GenParticle*>()};
  };
