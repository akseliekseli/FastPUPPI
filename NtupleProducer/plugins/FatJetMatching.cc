...
void FatJetMatching::flavorLabel(const l1t::PFJet* jet,
                                 const reco::GenParticleCollection& genParticles,
                                 double distR,
                                 unsigned int nBHadrons,
                                 unsigned int nCHadrons) {
  processed_.clear();
  clearResult();
  bool debug_print_genparts_table_ = false;
  if (debug_print_genparts_table_) {
    std::cout << "\n=======\nJet (energy, pT, eta, phi) = " << jet->energy() << ", " << jet->pt() << ", " << jet->eta()
              << ", " << jet->phi() << std::endl
              << std::endl;
    printGenInfoHeader();
  ...
  for (unsigned ipart = 0; ipart < genParticles.size(); ++ipart) {
    ...
    } else if (pdgid == ParticleID::p_H0 || pdgid == ParticleID::p_Hplus || pdgid == ParticleID::p_Hbsm ||
               pdgid == ParticleID::p_LQbsm || (!found_reslike_higgs && pdgid == ParticleID::p_h0)) {
      found_higgs = true;
  ...
  clearResult();
  qcd_label(nBHadrons, nCHadrons);
}

void FatJetMatching::printGenInfoHeader() const {
  using namespace std;
  cout << right << setw(6) << "#" << " " << setw(10) << "pdgId"
       << "  " << "Chg" << "  " << setw(10) << "Mass" << "  " << setw(48) << " Momentum" << left << "  " << setw(10)
       << "Mothers" << " " << setw(30) << "Daughters" << endl;
}
...
const reco::GenParticle* FatJetMatching::getHeavyHadronAncestor(const reco::GenParticle* particle) {
  auto pdgid = std::abs(particle->pdgId());
  int type;
  if ((pdgid >= 400 && pdgid < 500) || (pdgid >= 4000 && pdgid < 5000))
    type = 4;
  else if ((pdgid >= 500 && pdgid < 600) || (pdgid >= 5000 && pdgid < 6000))
    type = 5;
  else
    return nullptr;
  ...
  while (final->numberOfMothers()) {
    ...
    for (unsigned imom = 0; imom < final->numberOfMothers(); ++imom) {
      ...
      if (pdgid_m == type || (pdgid_m >= type * 100 && pdgid_m < (type + 1) * 100) ||
          (pdgid_m >= type * 1000 && pdgid_m < (type + 1) * 1000)) {
        final = dynamic_cast<const reco::GenParticle*>(final->mother(imom));
        break;
      }
      if (pdgid_m == ParticleID::p_proton || pdgid_m == ParticleID::p_t || pdgid_m == ParticleID::p_Z0 ||
          pdgid_m == ParticleID::p_Wplus || pdgid_m == ParticleID::p_h0) {
        ancestor = dynamic_cast<const reco::GenParticle*>(final->mother(imom));
        break;
      }
      if (imom == final->numberOfMothers() - 1)
        return nullptr;
    }
    if (ancestor)
      break;
...
std::pair<std::vector<const reco::GenParticle*>, int> FatJetMatching::getTauDaughters(
  ...
  for (const auto& dau : daughters) {
    ...
    if (pdgid == ParticleID::p_eminus)
      return std::make_pair(daughters, 0);
    if (pdgid == ParticleID::p_muminus)
      return std::make_pair(daughters, 1);
  }
  return std::make_pair(daughters, 2);  // hadronic mode
}

void FatJetMatching::top_label(const l1t::PFJet* jet,
  ...
  if (pdgid_0 == ParticleID::p_eminus)
    wdecay = W_ev;
  else if (pdgid_0 == ParticleID::p_muminus)
    wdecay = W_mv;
    ...
    if (tau_daus_info.second == 0)
      wdecay = W_tauev;
    else if (tau_daus_info.second == 1)
      wdecay = W_taumv;
  ...
  if (wdecay == W_cs || wdecay == W_qq) {
    ...
    if (dr_b < distR) {
      ...
      if ((wdecay == W_tauev || wdecay == W_taumv) && reco::deltaR(jet->p4(), tau_daus.at(0)->p4()) < distR) {
        if (wdecay == W_tauev)
          getResult().label = "Top_bWtauev";
        else if (wdecay == W_taumv)
          getResult().label = "Top_bWtaumv";
...
void FatJetMatching::w_label(const l1t::PFJet* jet, const reco::GenParticle* parton, double distR, bool is_from_top) {
  ...
  if ((wdecay == W_cs || wdecay == W_qq) &&
      (reco::deltaR(jet->p4(), wdaus.at(0)->p4()) >= distR || reco::deltaR(jet->p4(), wdaus.at(1)->p4()) >= distR)) {
    getResult().label = "Invalid";
    return;
  }
  ...
  if (!is_from_top) {
    if (wdecay == W_cs)
      getResult().label = "W_cs";
    else if (wdecay == W_qq)
      getResult().label = "W_qq";
    else if (wdecay == W_ev)
      getResult().label = "W_ev";
    else if (wdecay == W_mv)
      getResult().label = "W_mv";
    else if (wdecay == W_tauev)
      getResult().label = "W_tauev";
    else if (wdecay == W_taumv)
      getResult().label = "W_taumv";
    else if (wdecay == W_tauhv)
      getResult().label = "W_tauhv";
  } else {
    if (wdecay == W_cs)
      getResult().label = "Top_Wcs";
    else if (wdecay == W_qq)
      getResult().label = "Top_Wqq";
    else if (wdecay == W_ev)
      getResult().label = "Top_Wev";
    else if (wdecay == W_mv)
      getResult().label = "Top_Wmv";
    else if (wdecay == W_tauev)
      getResult().label = "Top_Wtauev";
    else if (wdecay == W_taumv)
      getResult().label = "Top_Wtaumv";
    else if (wdecay == W_tauhv)
      getResult().label = "Top_Wtauhv";
  }
...
void FatJetMatching::higgs_label(const l1t::PFJet* jet, const reco::GenParticle* parton, double distR) {
  ...
  if (hdecay == h_WW || hdecay == h_ZZ || hdecay == h_WHorZH) {
      ...
      for (unsigned j = 0; j < daufinal->numberOfDaughters(); ++j) {
        ...
        if (j == 0) {
          ...
          if (dpdgid >= ParticleID::p_d && dpdgid <= ParticleID::p_b)
            hVV_modes[idau] = v_had;
          else if (dpdgid == ParticleID::p_gamma)
            hVV_modes[idau] = v_aa;
          else
            hVV_modes[idau] = v_lep;
        }
    ...
    //    if (zdaus.size() >= 2)
    {
      ...
      if (dr_q1 < distR && dr_q2 < distR) {
        ...
        } else if ((pdgid_q1 == ParticleID::p_u && pdgid_q2 == ParticleID::p_u) ||
                   (pdgid_q1 == ParticleID::p_d && pdgid_q2 == ParticleID::p_d) ||
                   (pdgid_q1 == ParticleID::p_u && pdgid_q2 == ParticleID::p_d) ||
                   (pdgid_q1 == ParticleID::p_d && pdgid_q2 == ParticleID::p_u)) {
          getResult().label = "H_qq";
        } else if ((pdgid_q1 == ParticleID::p_b && pdgid_q2 == ParticleID::p_c) ||
                   (pdgid_q1 == ParticleID::p_c && pdgid_q2 == ParticleID::p_b)) {
          getResult().label = "H_bc";
        } else if ((pdgid_q1 == ParticleID::p_b && pdgid_q2 == ParticleID::p_s) ||
                   (pdgid_q1 == ParticleID::p_s && pdgid_q2 == ParticleID::p_b)) {
          getResult().label = "H_bs";
        } else if ((pdgid_q1 == ParticleID::p_c && pdgid_q2 == ParticleID::p_s) ||
                   (pdgid_q1 == ParticleID::p_s && pdgid_q2 == ParticleID::p_c)) {
    ...
    if (hdaus.size() == 2) {
      ...
      if (tau1_daus_info.second == 2 && dr_tau1 < distR) {
        // push the first tauh
        getResult().particles.insert(
            getResult().particles.end(), tau1_daus_info.first.begin(), tau1_daus_info.first.end());

        // inspect the second tau
        if ((tau2_daus_info.second == 0 || tau2_daus_info.second == 1) &&
            reco::deltaR(tau2_daus_info.first.at(0)->p4(), jet->p4()) < distR) {
          ...
          getResult().particles.insert(
              getResult().particles.end(), tau2_daus_info.first.begin(), tau2_daus_info.first.end());
    ...
    if (hdaus.size() == 2) {
      ...
      if (dr_q < distR) {
        ...
        if (std::abs(hdaus.at(0)->pdgId()) == ParticleID::p_b) {
          ...
          qtype = "s";
        } else if (std::abs(hdaus.at(0)->pdgId()) == ParticleID::p_u ||
                   std::abs(hdaus.at(0)->pdgId()) == ParticleID::p_d) {
          qtype = "q";
        ...
        if ((tau_daus_info.second == 0 || tau_daus_info.second == 1) &&
            reco::deltaR(tau_daus_info.first.at(0)->p4(), jet->p4()) < distR) {
          ...
          getResult().particles.insert(
              getResult().particles.end(), tau_daus_info.first.begin(), tau_daus_info.first.end());
...
void FatJetMatching::higgs_WW_label(const l1t::PFJet* jet,
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    if (debug_) {
      ...
      cout << "  deltaR(jet, dau1)  : " << reco::deltaR(jet->p4(), daus[0]->p4())
           << "  dau/tau-dau is matched: " << daus_matched[i * 2] << endl;
      cout << "  deltaR(jet, dau2)  : " << reco::deltaR(jet->p4(), daus[1]->p4())
           << "  dau/tau-dau is matched: " << daus_matched[i * 2 + 1] << endl;
  ...
  // then will be qqlv case
    ...
    if (daus_matched[0] && daus_matched[1] && daus_matched[2]) {
      if (wsdecay[1] == W_ev)
        getResult().label = (wsdecay[0] == W_cs) ? "H_WW_csev" : "H_WW_qqev";
      else if (wsdecay[1] == W_mv)
        getResult().label = (wsdecay[0] == W_cs) ? "H_WW_csmv" : "H_WW_qqmv";
      else if (wsdecay[1] == W_tauev)
        getResult().label = (wsdecay[0] == W_cs) ? "H_WW_cstauev" : "H_WW_qqtauev";
      else if (wsdecay[1] == W_taumv)
        getResult().label = (wsdecay[0] == W_cs) ? "H_WW_cstaumv" : "H_WW_qqtaumv";
      else if (wsdecay[1] == W_tauhv)
        getResult().label = (wsdecay[0] == W_cs) ? "H_WW_cstauhv" : "H_WW_qqtauhv";
    }
  }
}

void FatJetMatching::higgs_ZZ_label(const l1t::PFJet* jet,
                                    std::vector<const reco::GenParticle*>& hVV_daughters,
                                    double distR) {
    ...
    if (std::abs(daus[0]->pdgId()) >= ParticleID::p_d && std::abs(daus[0]->pdgId()) <= ParticleID::p_nu_mu) {
      ...
      if (std::abs(daus[0]->pdgId()) == ParticleID::p_b && std::abs(daus[1]->pdgId()) == ParticleID::p_b) {
        ...
        zsdecay[i] = Z_qq;
                 ...
                 std::abs(daus[1]->pdgId()) == ParticleID::p_eminus) {
        zsdecay[i] = Z_ee;
      } else if (std::abs(daus[0]->pdgId()) == ParticleID::p_muminus &&
                 std::abs(daus[1]->pdgId()) == ParticleID::p_muminus) {
        zsdecay[i] = Z_mm;
      }
      // check if the quarks are matched to the jet
      for (int j = 0; j < 2; ++j) {
        if (reco::deltaR(jet->p4(), daus[j]->p4()) < distR) {
          ...
          getResult().particles.push_back(daus[j]);
               ...
               std::abs(daus[1]->pdgId()) == ParticleID::p_tauminus) {
      // Z->tautau
    ...
    if (debug_) {
      ...
      if (zsdecay[i] == Z_tauhtaue || zsdecay[i] == Z_tauhtaum || zsdecay[i] == Z_tauhtauh) {
        for (int j = 0; j < 2; j++) {
          ...
          cout << "    tau daughters: " << endl;
          for (auto dau : tau_daus) {
            cout << "      pdgid: " << dau->pdgId() << "  deltaR(jet, dau): " << reco::deltaR(jet->p4(), dau->p4())
                 << endl;
          }
        }
      }
  ...
  for (int i = 0; i < 2; ++i) {
    if (zsdecay[i] == Z_bb)
      matched_parts_str += (daus_matched[i * 2] && daus_matched[i * 2 + 1]) ? "bb" : "b";
    else if (zsdecay[i] == Z_cc)
      matched_parts_str += (daus_matched[i * 2] && daus_matched[i * 2 + 1]) ? "cc" : "c";
    else if (zsdecay[i] == Z_ss)
      matched_parts_str += (daus_matched[i * 2] && daus_matched[i * 2 + 1]) ? "ss" : "s";
    else if (zsdecay[i] == Z_qq)
      matched_parts_str += (daus_matched[i * 2] && daus_matched[i * 2 + 1]) ? "qq" : "q";
    else if (zsdecay[i] == Z_ee)
      matched_parts_str += (daus_matched[i * 2] && daus_matched[i * 2 + 1]) ? "ee" : "e";
    else if (zsdecay[i] == Z_mm)
      matched_parts_str += (daus_matched[i * 2] && daus_matched[i * 2 + 1]) ? "mm" : "m";
    else if (zsdecay[i] == Z_tauhtaue)
      matched_parts_str += (daus_matched[i * 2] && daus_matched[i * 2 + 1]) ? "tauhtaue" : "NULL";
    else if (zsdecay[i] == Z_tauhtaum)
      matched_parts_str += (daus_matched[i * 2] && daus_matched[i * 2 + 1]) ? "tauhtaum" : "NULL";
    else if (zsdecay[i] == Z_tauhtauh)
      matched_parts_str += (daus_matched[i * 2] && daus_matched[i * 2 + 1]) ? "tauhtauh" : "NULL";
  }
  // desinate the label
  std::map<std::vector<std::string>, std::string> acceptable_strs_map = {
      ...
      {{"bbee", "eebb"}, "H_ZZ_bbee"},
      {{"bbmm", "mmbb"}, "H_ZZ_bbmm"},
      {{"bbtauhtaue", "tauhtauebb"}, "H_ZZ_bbtauhtaue"},
      {{"bbtauhtaum", "tauhtaumbb"}, "H_ZZ_bbtauhtaum"},
      {{"bbtauhtauh", "tauhtauhbb"}, "H_ZZ_bbtauhtauh"},
      {{"bbb"}, "H_ZZ_bbb"},
      {{"bbc", "cbb"}, "H_ZZ_bbc"},
      {{"bbs", "sbb"}, "H_ZZ_bbs"},
      {{"bbq", "qbb"}, "H_ZZ_bbq"},
      ...
      {{"ccee", "eecc"}, "H_ZZ_ccee"},
      {{"ccmm", "mmcc"}, "H_ZZ_ccmm"},
      {{"cctauhtaue", "tauhtauecc"}, "H_ZZ_cctauhtaue"},
      {{"cctauhtaum", "tauhtaumcc"}, "H_ZZ_cctauhtaum"},
      {{"cctauhtauh", "tauhtauhcc"}, "H_ZZ_cctauhtauh"},
      {{"ccb", "bcc"}, "H_ZZ_ccb"},
      {{"ccc"}, "H_ZZ_ccc"},
      {{"ccs", "scc"}, "H_ZZ_ccs"},
      {{"ccq", "qcc"}, "H_ZZ_ccq"},
      ...
      {{"ssee", "eess"}, "H_ZZ_ssee"},
      {{"ssmm", "mmss"}, "H_ZZ_ssmm"},
      {{"sstauhtaue", "tauhtauess"}, "H_ZZ_sstauhtaue"},
      {{"sstauhtaum", "tauhtaumss"}, "H_ZZ_sstauhtaum"},
      {{"sstauhtauh", "tauhtauhss"}, "H_ZZ_sstauhtauh"},
      {{"ssb", "bss"}, "H_ZZ_ssb"},
      {{"ssc", "css"}, "H_ZZ_ssc"},
      {{"sss"}, "H_ZZ_sss"},
      {{"ssq", "qss"}, "H_ZZ_ssq"},
      ...
      {{"qqee", "eeqq"}, "H_ZZ_qqee"},
      {{"qqmm", "mmqq"}, "H_ZZ_qqmm"},
      {{"qqtauhtaue", "tauhtaueqq"}, "H_ZZ_qqtauhtaue"},
      {{"qqtauhtaum", "tauhtaumqq"}, "H_ZZ_qqtauhtaum"},
      {{"qqtauhtauh", "tauhtauhqq"}, "H_ZZ_qqtauhtauh"},
      {{"qqb", "bqq"}, "H_ZZ_qqb"},
      {{"qqc", "cqq"}, "H_ZZ_qqc"},
      {{"qqs", "sqq"}, "H_ZZ_qqs"},
      {{"qqq"}, "H_ZZ_qqq"},
      ...
      {{"mms", "smm"}, "H_ZZ_smm"},
      {{"mmq", "qmm"}, "H_ZZ_qmm"},
      {{"tauhtaueb", "btauhtaue"}, "H_ZZ_btauhtaue"},
      {{"tauhtauec", "ctauhtaue"}, "H_ZZ_ctauhtaue"},
      {{"tauhtaues", "stauhtaue"}, "H_ZZ_stauhtaue"},
      {{"tauhtaueq", "qtauhtaue"}, "H_ZZ_qtauhtaue"},
      {{"tauhtaumb", "btauhtaum"}, "H_ZZ_btauhtaum"},
      {{"tauhtaumc", "ctauhtaum"}, "H_ZZ_ctauhtaum"},
      {{"tauhtaums", "stauhtaum"}, "H_ZZ_stauhtaum"},
      {{"tauhtaumq", "qtauhtaum"}, "H_ZZ_qtauhtaum"},
      {{"tauhtauhb", "btauhtauh"}, "H_ZZ_btauhtauh"},
      {{"tauhtauhc", "ctauhtauh"}, "H_ZZ_ctauhtauh"},
      {{"tauhtauhs", "stauhtauh"}, "H_ZZ_stauhtauh"},
      {{"tauhtauhq", "qtauhtauh"}, "H_ZZ_qtauhtauh"},
  };
...
void FatJetMatching::higgs_WHorZH_label(const l1t::PFJet* jet,
                                        std::vector<const reco::GenParticle*>& hVV_daughters,
                                        double distR) {
  ...
  for (int i = 0; i < 2; ++i) {
    const reco::GenParticle* daus[2] = {hVV_daughters.at(i * 2), hVV_daughters.at(i * 2 + 1)};
    if ((std::abs(daus[0]->pdgId()) >= ParticleID::p_d && std::abs(daus[0]->pdgId()) <= ParticleID::p_nu_mu) ||
        std::abs(daus[0]->pdgId()) == ParticleID::p_g || std::abs(daus[0]->pdgId()) == ParticleID::p_gamma) {
      // non-tau cases
      if (std::abs(daus[0]->pdgId()) == ParticleID::p_gamma && std::abs(daus[1]->pdgId()) == ParticleID::p_gamma) {
        ...
        std::swap(daus[0], daus[1]);
        rsdecay[i] = W_cs;
      } else if (std::abs(daus[0]->pdgId()) == ParticleID::p_b &&
                 (std::abs(daus[1]->pdgId()) == ParticleID::p_u || std::abs(daus[1]->pdgId()) == ParticleID::p_d)) {
        rsdecay[i] = W_bq;
      } else if (std::abs(daus[1]->pdgId()) == ParticleID::p_b &&
                 (std::abs(daus[0]->pdgId()) == ParticleID::p_u || std::abs(daus[0]->pdgId()) == ParticleID::p_d)) {
        std::swap(daus[0], daus[1]);
        rsdecay[i] = W_bq;
      } else if (std::abs(daus[0]->pdgId()) == ParticleID::p_c &&
                 (std::abs(daus[1]->pdgId()) == ParticleID::p_u || std::abs(daus[1]->pdgId()) == ParticleID::p_d)) {
        rsdecay[i] = W_cq;
      } else if (std::abs(daus[1]->pdgId()) == ParticleID::p_c &&
                 (std::abs(daus[0]->pdgId()) == ParticleID::p_u || std::abs(daus[0]->pdgId()) == ParticleID::p_d)) {
        std::swap(daus[0], daus[1]);
        rsdecay[i] = W_cq;
      } else if (std::abs(daus[0]->pdgId()) == ParticleID::p_s &&
                 (std::abs(daus[1]->pdgId()) == ParticleID::p_u || std::abs(daus[1]->pdgId()) == ParticleID::p_d)) {
        rsdecay[i] = W_sq;
      } else if (std::abs(daus[1]->pdgId()) == ParticleID::p_s &&
                 (std::abs(daus[0]->pdgId()) == ParticleID::p_u || std::abs(daus[0]->pdgId()) == ParticleID::p_d)) {
        std::swap(daus[0], daus[1]);
        rsdecay[i] = W_sq;
        ...
        rsdecay[i] = Z_mm;
                 ...
                 std::abs(daus[1]->pdgId()) == ParticleID::p_nu_e) {
        rsdecay[i] = W_ev;
      } else if (std::abs(daus[0]->pdgId()) == ParticleID::p_muminus &&
                 std::abs(daus[1]->pdgId()) == ParticleID::p_nu_mu) {
        rsdecay[i] = W_mv;
      }
      // check if the quarks are matched to the jet
      for (int j = 0; j < 2; ++j) {
        ...
        if (tau2_daus_info.second == 0 || tau2_daus_info.second == 1) {
          ...
          if (reco::deltaR(jet->p4(), daus[1]->p4()) < distR) {
            ...
            getResult().particles.insert(
               ...
               std::abs(daus[1]->pdgId()) == ParticleID::p_nu_tau) {
      // W->tauv
      auto tau1_daus_info = getTauDaughters(daus[0]);
    ...
    if (debug_) {
      ...
      cout << "  deltaR(jet, dau2)  : " << reco::deltaR(jet->p4(), daus[1]->p4())
           << "  dau/tau-dau is matched: " << daus_matched[i * 2 + 1] << endl;
      if (rsdecay[i] == Z_tauhtaue || rsdecay[i] == Z_tauhtaum || rsdecay[i] == Z_tauhtauh || rsdecay[i] == W_tauev ||
          rsdecay[i] == W_taumv || rsdecay[i] == W_tauhv) {
        int num_tau = 0;
        if (rsdecay[i] == Z_tauhtaue || rsdecay[i] == Z_tauhtaum || rsdecay[i] == Z_tauhtauh)
          num_tau = 2;
        else if (rsdecay[i] == W_tauev || rsdecay[i] == W_taumv || rsdecay[i] == W_tauhv)
          num_tau = 1;
  ...
  for (int i = 0; i < 2; ++i) {
    if (rsdecay[i] == Z_bb)
      matched_parts_str += (daus_matched[i * 2] && daus_matched[i * 2 + 1]) ? "bb" : "b";
    else if (rsdecay[i] == Z_cc)
      matched_parts_str += (daus_matched[i * 2] && daus_matched[i * 2 + 1]) ? "cc" : "c";
    else if (rsdecay[i] == Z_ss)
      matched_parts_str += (daus_matched[i * 2] && daus_matched[i * 2 + 1]) ? "ss" : "s";
    else if (rsdecay[i] == Z_qq)
      matched_parts_str += (daus_matched[i * 2] && daus_matched[i * 2 + 1]) ? "qq" : "q";
    else if (rsdecay[i] == W_bc)
      matched_parts_str += (daus_matched[i * 2] && daus_matched[i * 2 + 1]) ? "bc" : daus_matched[i * 2] ? "b" : "c";
    else if (rsdecay[i] == W_cs)
      matched_parts_str += (daus_matched[i * 2] && daus_matched[i * 2 + 1]) ? "cs" : daus_matched[i * 2] ? "c" : "s";
    else if (rsdecay[i] == W_bq)
      matched_parts_str += (daus_matched[i * 2] && daus_matched[i * 2 + 1]) ? "bq" : daus_matched[i * 2] ? "b" : "q";
    else if (rsdecay[i] == W_cq)
      matched_parts_str += (daus_matched[i * 2] && daus_matched[i * 2 + 1]) ? "cq" : daus_matched[i * 2] ? "c" : "q";
    else if (rsdecay[i] == W_sq)
      matched_parts_str += (daus_matched[i * 2] && daus_matched[i * 2 + 1]) ? "sq" : daus_matched[i * 2] ? "s" : "q";
    else if (rsdecay[i] == Z_gg)
      matched_parts_str += (daus_matched[i * 2] && daus_matched[i * 2 + 1]) ? "gg" : "g";
    else if (rsdecay[i] == H_aa)
      matched_parts_str += (daus_matched[i * 2] && daus_matched[i * 2 + 1]) ? "aa" : "a";
    else if (rsdecay[i] == Z_ee)
      matched_parts_str += (daus_matched[i * 2] && daus_matched[i * 2 + 1]) ? "ee" : "e";
    else if (rsdecay[i] == Z_mm)
      matched_parts_str += (daus_matched[i * 2] && daus_matched[i * 2 + 1]) ? "mm" : "m";
    else if (rsdecay[i] == W_ev)
      matched_parts_str += (daus_matched[i * 2]) ? "e" : "NULL";
    else if (rsdecay[i] == W_mv)
      matched_parts_str += (daus_matched[i * 2]) ? "m" : "NULL";
    else if (rsdecay[i] == Z_tauhtaue)
      matched_parts_str +=
          (daus_matched[i * 2] && daus_matched[i * 2 + 1]) ? "tauhtaue" : "NULL";  // NULL is not possible
    else if (rsdecay[i] == Z_tauhtaum)
      matched_parts_str += (daus_matched[i * 2] && daus_matched[i * 2 + 1]) ? "tauhtaum" : "NULL";
    else if (rsdecay[i] == Z_tauhtauh)
      matched_parts_str += (daus_matched[i * 2] && daus_matched[i * 2 + 1]) ? "tauhtauh" : "NULL";
    else if (rsdecay[i] == W_tauev)
      matched_parts_str += (daus_matched[i * 2]) ? "taue" : "NULL";
    else if (rsdecay[i] == W_taumv)
      matched_parts_str += (daus_matched[i * 2]) ? "taum" : "NULL";
    else if (rsdecay[i] == W_tauhv)
      matched_parts_str += (daus_matched[i * 2]) ? "tauh" : "NULL";
  }
  // desinate the label
  std::map<std::vector<std::string>, std::string> acceptable_strs_map = {
      {{"aabb"}, "H_HVext_aabb"},
      ...
      {{"aae"}, "H_HVext_aae"},
      {{"aam"}, "H_HVext_aam"},
      {{"aataue"}, "H_HVext_aataue"},
      {{"aataum"}, "H_HVext_aataum"},
      {{"aatauh"}, "H_HVext_aatauh"},
      {{"abb"}, "H_HVext_abb"},
      {{"acc"}, "H_HVext_acc"},
      {{"ass"}, "H_HVext_ass"},
      {{"aqq"}, "H_HVext_aqq"},
      ...
      {{"aee"}, "H_HVext_aee"},
      {{"amm"}, "H_HVext_amm"},
      {{"atauhtaue"}, "H_HVext_atauhtaue"},
      {{"atauhtaum"}, "H_HVext_atauhtaum"},
      {{"atauhtauh"}, "H_HVext_atauhtauh"},
  };
  for (auto it = acceptable_strs_map.begin(); it != acceptable_strs_map.end(); ++it) {
    auto& acceptable_strs = it->first;
    std::string& label = it->second;
...
void FatJetMatching::qcd_label(unsigned int nBHadrons, unsigned int nCHadrons) {
  if (nBHadrons >= 2) {
