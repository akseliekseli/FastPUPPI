...
JetNTuplizer::JetNTuplizer(const edm::ParameterSet& iConfig)
    : genjets_(consumes<std::vector<reco::GenJet>>(iConfig.getParameter<edm::InputTag>("genJets"))),
      ...
      writeJetLabel_(iConfig.getParameter<bool>("writeJetLabel")),
      jetLabelRadius_(iConfig.getParameter<double>("jetLabelRadius")),
      jetLabelBranch_(iConfig.getParameter<std::string>("jetLabelBranch")) {
...
void JetNTuplizer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  for (size_t i = 0; i < jetv_l1.size(); i++) {
    ...
    jet_pt_log_ =
        L1TSC4NGJet::inputtype(l1ct::log_with_shift<l1ct::pt_t, L1TSC4NGJet::log_pt_t, LOG_LUT_SIZE>(ctJet.hwPt));
    ...
    if (doOfflineInfo_) {
      ...
      if (pos_matched_offline > -1) {
        ...
        if (fabs(jetv_offline[pos_matched_offline]->eta()) < 2.4) {
          jet_offline_btag_deepcsv_probb_ =
              jetv_offline[pos_matched_offline]->bDiscriminator("pfDeepCSVJetTags:probb") +
              jetv_offline[pos_matched_offline]->bDiscriminator("pfDeepCSVJetTags:probbb");
          jet_offline_btag_deepcsv_probc_ = jetv_offline[pos_matched_offline]->bDiscriminator("pfDeepCSVJetTags:probc");
          jet_offline_btag_deepcsv_probudsg_ =
              jetv_offline[pos_matched_offline]->bDiscriminator("pfDeepCSVJetTags:probudsg");

          jet_offline_btag_deepjet_probb_ =
              jetv_offline[pos_matched_offline]->bDiscriminator("pfDeepFlavourJetTags:probb") +
              jetv_offline[pos_matched_offline]->bDiscriminator("pfDeepFlavourJetTags:probbb") +
              jetv_offline[pos_matched_offline]->bDiscriminator("pfDeepFlavourJetTags:problepb");
          jet_offline_btag_deepjet_probc_ =
              jetv_offline[pos_matched_offline]->bDiscriminator("pfDeepFlavourJetTags:probc");
          jet_offline_btag_deepjet_probuds_ =
              jetv_offline[pos_matched_offline]->bDiscriminator("pfDeepFlavourJetTags:probuds");
          jet_offline_btag_deepjet_probg_ =
              jetv_offline[pos_matched_offline]->bDiscriminator("pfDeepFlavourJetTags:probg");

          jet_offline_btag_pnet_probb_ = (jetv_offline[pos_matched_offline]->bDiscriminator(
              "pfParticleNetFromMiniAODAK4PuppiCentralJetTags:probb"));
          jet_offline_btag_pnet_probc_ = (jetv_offline[pos_matched_offline]->bDiscriminator(
              "pfParticleNetFromMiniAODAK4PuppiCentralJetTags:probc"));
          jet_offline_btag_pnet_probuds_ = (jetv_offline[pos_matched_offline]->bDiscriminator(
              "pfParticleNetFromMiniAODAK4PuppiCentralJetTags:probuds"));
          jet_offline_btag_pnet_probg_ = (jetv_offline[pos_matched_offline]->bDiscriminator(
              "pfParticleNetFromMiniAODAK4PuppiCentralJetTags:probg"));
          jet_offline_btag_pnet_probmu_ = (jetv_offline[pos_matched_offline]->bDiscriminator(
              "pfParticleNetFromMiniAODAK4PuppiCentralJetTags:probmu"));
          jet_offline_btag_pnet_probele_ = (jetv_offline[pos_matched_offline]->bDiscriminator(
              "pfParticleNetFromMiniAODAK4PuppiCentralJetTags:probele"));
          jet_offline_btag_pnet_probtaup_ = (jetv_offline[pos_matched_offline]->bDiscriminator(
                                                "pfParticleNetFromMiniAODAK4PuppiCentralJetTags:probtaup1h0p")) +
                                            (jetv_offline[pos_matched_offline]->bDiscriminator(
          ...
          jet_offline_btag_pnet_probtaum_ = (jetv_offline[pos_matched_offline]->bDiscriminator(
        ...
        if (fabs(jetv_offline[pos_matched_offline]->eta()) > 2.4) {
          jet_offline_btag_pnetForward_probq_ = (jetv_offline[pos_matched_offline]->bDiscriminator(
              "pfParticleNetFromMiniAODAK4PuppiForwardJetTags:probq"));
          jet_offline_btag_pnetForward_probg_ = (jetv_offline[pos_matched_offline]->bDiscriminator(
        ...
        TVector3 jet_direction_off(jetv_offline[pos_matched_offline]->momentum().Unit().x(),
                                   jetv_offline[pos_matched_offline]->momentum().Unit().y(),
                                   jetv_offline[pos_matched_offline]->momentum().Unit().z());
        ...
        for (unsigned int j = 0; j < jetv_offline[pos_matched_offline]->numberOfDaughters(); j++) {
          ...
          if (packed) {
            ...
            jet_offline_pfcand_quality_.push_back(packed->hasTrackDetails() ? packed->pseudoTrack().qualityMask()
            ...
            if ((abs(packed->pdgId()) != 22) && (abs(packed->pdgId()) != 11) && (abs(packed->pdgId()) != 13)) {
              if (packed->charge() != 0) {
                ...
                if ((fabs(packed->pdgId()) < 11) || (fabs(packed->pdgId()) > 13)) {
                  ...
                  std::cout << "WARNING: charged lepton found in jet constituents! This should not happen!"
                            << std::endl;
    ...
    for (size_t ipfcand = 0; ipfcand < vectorOfConstituents.size(); ipfcand++) {
      ...
      L1TSC4NGJet::inputtype inv_jet_pt =
          L1TSC4NGJet::inputtype(l1ct::invert_with_shift<l1ct::pt_t, L1TSC4NGJet::inv_pt_t, INV_LUT_SIZE>(
              jet_pt_));  // divide by HW jet pt, which should be without JEC!

      constexpr int LOG_LUT_SIZE = 256;
      L1TSC4NGJet::inputtype log_cand_pt =
          L1TSC4NGJet::inputtype(l1ct::log_with_shift<l1ct::pt_t, L1TSC4NGJet::log_pt_t, LOG_LUT_SIZE>(puppicand.hwPt));
      ...
      jet_puppicand_z0.push_back(L1TSC4NGJet::inputtype(puppicand.hwId.charged())
                                     ? L1TSC4NGJet::inputtype(puppicand.hwZ0())
                                     : L1TSC4NGJet::inputtype(0.0));
      jet_puppicand_dxy.push_back(L1TSC4NGJet::inputtype(puppicand.hwId.charged())
                                      ? L1TSC4NGJet::inputtype(puppicand.hwDxy())
                                      : L1TSC4NGJet::inputtype(0.0));
      jet_puppicand_is_filled.push_back(L1TSC4NGJet::inputtype(1.0));
      jet_puppicand_puppi_weight.push_back(L1TSC4NGJet::inputtype(puppicand.hwId.neutral())
                                               ? L1TSC4NGJet::inputtype(puppicand.hwPuppiW())
                                               : L1TSC4NGJet::inputtype(0.0));
      jet_puppicand_emid.push_back(L1TSC4NGJet::inputtype(puppicand.hwId.neutral())
                                       ? L1TSC4NGJet::inputtype(puppicand.hwEmID())
                                       : L1TSC4NGJet::inputtype(0.0));
      jet_puppicand_quality.push_back(L1TSC4NGJet::inputtype(puppicand.hwId.charged())
                                          ? L1TSC4NGJet::inputtype(puppicand.hwTkQuality())
                                          : L1TSC4NGJet::inputtype(0.0));
  ...
  if (!iEvent.isRealData()) {
    ...
    if (genParticles.isValid()) {
      ...
      for (auto gens_iter = genParticles->begin(); gens_iter != genParticles->end(); ++gens_iter) {
        ...
        uint countH = 0;
        if ((abs(gens_iter->pdgId()) == 25) and gens_iter->isLastCopy() and
            gens_iter->statusFlags().fromHardProcess() and countH < 2) {
          countH++;
